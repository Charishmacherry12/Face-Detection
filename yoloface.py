...
parser.add_argument('--image', type=str, default='./samples/Five.jpeg',
...
def _main():
    ...
    img = cv2.imread('./samples/Five.jpeg')
    path  = r'C:\Users\potnu\PycharmProjects\main project\Extracted Faces'
    for i in range(len(faces)):
        cropped_img = cv2.resize(bounding_box_img(img, faces[i]), (100, 100))
        cv2.imshow(str(i+1),cropped_img)
        nm = str(i+1) + ".jpg"
        cv2.imwrite(os. path. join(path , nm),cropped_img)
        cv2.waitKey(0)
